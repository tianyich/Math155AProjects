...
unsigned int phShaderTransparent;
...
void setup_phong_shaders() {
    ...
    unsigned int shader_VPT = GlShaderMgr::CompileShader("vertexShader_PhongPhong");
    unsigned int shader_FPT = GlShaderMgr::CompileShader("myTransparentShader");
    unsigned int shaders_PT[2] = { shader_VPT, shader_FPT };
    phShaderTransparent = GlShaderMgr::LinkShaderProgram(2, shaders_PP);
    phRegisterShaderProgram(phShaderPhongPhong);
}
