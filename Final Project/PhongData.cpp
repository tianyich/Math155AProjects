...
VectorR3 myLightPositions[2] = {
    VectorR3(0.0, 15.0, 0.0),
    VectorR3(0.0, 1.0, -3.0f),
};
...
void MySetupLights()
{
    ...
    // First light (light #0).
    myLights[0].AmbientColor.Set(0.7, 0.7, 0.7);    
    myLights[0].DiffuseColor.Set(0.8,0.8,0.8);      
    myLights[0].SpecularColor.Set(0.9, 0.9, 0.9);   
    myLights[0].IsEnabled = true;                   

    myLights[1].AmbientColor.Set(0.4, 0.4, 0.1);
    myLights[1].DiffuseColor.Set(0.7, 0.7, 0.2);
    myLights[1].SpecularColor.Set(0.1, 0.1, 0.0);
    myLights[1].IsEnabled = true;

    myLights[3].DiffuseColor.Set(0.6, 0.6, 0.6);      
    myLights[3].SpotCosCutoff = 0.95f;
    myLights[3].IsSpotLight = true;
    myLights[3].SpotExponent = 0.8f;
...
void LoadAllLights() 
{
    ...
    myLights[3].SetPosition(viewMatrix, VectorR3(0.0, 6.0, 2.0));
    myLights[3].SetSpotlightDirection(viewMatrix, VectorR3(0.0, -1.0, 0.0));
    myLights[3].LoadIntoShaders(2);
