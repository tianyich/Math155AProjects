...
double animateIncrement = -1.0;   // Make bigger to speed up animation, smaller to slow it down.
double currentTime = 0.0;         // Current "time" for the animation.
double bakingTime = 100.0;
...
const double Zmin = -12.0, Zmax = 10.0;   // Control z dimensions of the viewable scene

// zDistance equals the initial distance from the camera to the z = Zmax plane
const double zDistance = 40.0;              // Make this value larger or smaller to affect field of view.
...
void myRenderScene() {
    ...
    if (currentTime != 0.0 && currentTime != maxTime) {
    ...
    if (currentTime == maxTime) {
        ...
        bakingTime = (bakingTime < maxTime) ? bakingTime + 0.2 : maxTime;
        myLights[3].IsEnabled = false;
    }
    LoadAllLights();
...
void cursor_pos_callback(GLFWwindow* window,double x, double y) {
    if (mouseMode) {
        ...
        viewAzimuth = Min(viewAzimuth + 0.001 * (height / 2 - y), PIhalves - 0.05);
...
int main() {
    ...
    printf("Press '9' to adjust the view direction with mouse.\n");
    printf("Press 'O' to open or close the oven door\n");
    printf("Press Mouse buttons to switch to new dough when oven door is open\n");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glfwWaitEventsTimeout(1.0/120.0);	    // Use this to animate at 60 frames/sec (timing is NOT reliable)
